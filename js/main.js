...
(function(html) {
    ...
    * ------------------------------------------------------ */ 
    ...
    const lightbox = GLightbox({
        ...
        loop: false,
        autoplayVideos: true, // no autoplay
        plyr: {
            controls: [] // hides play, volume, and settings buttons
        },
        svg: {
    ...
    // Optional: ensure sound is on when video starts
    lightbox.on('slide_changed', ({ player }) => {
        if (player && player.media) {
            player.media.muted = false; // keep sound
            player.media.play();         // play after user click
        }
    });
